...
- Modified the `select_reference` function in the `snvphylnfc` workflow to allow for users to include a single reference genome in the samplesheet (without using the `--reference_sample_id` parameter). [PR #43](https://github.com/phac-nml/snvphylnfc/pull/43)
